...
// The cinode_backtrace_info_t is used for keeping the
// current state of the backtrace to be stored later on
// logsegment expire.  Constructing a cinode_backtrace_info_t
...
void LogSegment::queue_backtrace_update(CInode *inode, int64_t location, int64_t pool) {
    ...
    new cinode_backtrace_info_t(location, inode, this, pool);
}

void LogSegment::remove_pending_backtraces(inodeno_t ino, int64_t pool) {
  elist<cinode_backtrace_info_t*>::iterator i = update_backtraces.begin();
  while(!i.end()) {
    ++i;
...
unsigned LogSegment::encode_parent_mutation(ObjectOperation& m, cinode_backtrace_info_t *info)
{
  bufferlist parent;
...
struct C_LogSegment_StoredBacktrace : public Context {
  ...
  Context *fin;
  C_LogSegment_StoredBacktrace(LogSegment *l, cinode_backtrace_info_t *c,
...
void LogSegment::store_backtrace_update(MDS *mds, cinode_backtrace_info_t *info, Context *fin)
{
  ObjectOperation m;
...
void LogSegment::_stored_backtrace(cinode_backtrace_info_t *info, Context *fin)
{
  delete info;
  if (fin) {
    fin->finish(0);
